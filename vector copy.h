...
public:
    ...
    Vector(size_t v_size, T* values) { //initialize a vector. Can only hold up to CHUNK (10)
        data_ptr = new T[CHUNK];
        capacity = CHUNK;
        n_elems = 0;
        bool include = false;
    ...
    // Accessors
    T front() const {     // Return the T in position 0, if any
    ...
    void push_back(T item) {       // Append a new element at the end
    ...
    void insert(size_t pos, T item) {     // Shuffle items right to make room for new element
        if ((pos <0) || (pos >= n_elems)) 
    ...
    T* begin() {   // Return a pointer to 1st element, or nullptr if n_elems == 0
        T* p = (n_elems == 0)? nullptr:data_ptr;
        return p;
    };
    
    T* end() {     // Return a pointer to 1 past last element, or nullptr if n_elems == 0
        T* p = (n_elems == 0)? nullptr: (data_ptr + n_elems - 1);
        return p;
    };
