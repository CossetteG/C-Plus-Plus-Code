/* this is the raw file as I wrote and tested in JDoodle because Visual Studio Code was having 
trouble. This is like the first draft. As you can see, it can very easily be split into vector.h 
and vector.cpp. The testing in both of my test files is not set up according to the rubric becuase 
I was also using it to make sure my vector had the correct values in it and not just that it 
didn't throw an error and also I didn't notice the framework file that was in canvas until it was 
too late. 
*/
// #ifndef VECTOR_H
// #define VECTOR_H
